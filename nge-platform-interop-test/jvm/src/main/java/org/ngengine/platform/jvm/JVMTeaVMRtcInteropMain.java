...
public class JVMTeaVMRtcInteropMain {
    ...
    public static void main(String[] args) throws Exception {
        ...
        CountDownLatch browserReadyLatch = new CountDownLatch(1);
        ...
        try {
                        ...
                        for (JsonElement el : messages) {
                            ...
                            if ("answer".equals(type)) {
                                ...
                                try {
                                    ...
                                    // Browser candidates can occasionally be emitted in forms libdatachannel rejects.
                            ...
                            } else if ("browser-ready".equals(type)) {
                                browserReadyLatch.countDown();
                            } else if ("browser-failed".equals(type)) {
                                listenerError.compareAndSet(
                                    null,
                                    new IllegalStateException("Browser harness failed: " + msg.get("error").getAsString())
                                );
                            }
                        }
                        Thread.sleep(50);
            ...
            }, "jvm-teavm-interop-poller");
            poller.setDaemon(true);
            poller.start();

            String offer = transport.createChannel(RTCTransport.DEFAULT_CHANNEL, "jvm-teavm-proto", true, true, -1, null).await();
            ...
            awaitNoListenerError(listenerError, "waiting for browser answer");
            if (!answerLatch.await(15, TimeUnit.SECONDS)) {
                throw new IllegalStateException("Timed out waiting for browser SDP answer");
            }
            transport.connect(answerSdp.get()).await();

            awaitNoListenerError(listenerError, "waiting for RTC connected");
            if (!connectedLatch.await(15, TimeUnit.SECONDS)) {
                throw new IllegalStateException("Timed out waiting for RTC connected event");
            }

            RTCDataChannel channel = awaitDefaultChannel(transport, 15, TimeUnit.SECONDS);
            channel.ready().await();
            if (!browserReadyLatch.await(10, TimeUnit.SECONDS)) {
            ...
            while (System.nanoTime() < deadline && transport.getDataChannel(RTCTransport.DEFAULT_CHANNEL) != null) {
                Thread.sleep(20);
            }
