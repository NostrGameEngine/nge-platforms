...
function makeServer() {
  return http.createServer(async (req, res) => {
    try {
      ...
      const file = resolvePath(reqUrl.pathname === '/' ? '/test-harness/rtc-jvm-interop.html' : reqUrl.pathname);
      if (!file) {
...
async function main() {
  ...
  const pageUrl = `http://127.0.0.1:${port}/test-harness/rtc-jvm-interop.html?signalBase=${encodeURIComponent(signalBase)}`;
  ...
  if (browserOut?.consoleMessages?.length) {
    process.stderr.write(`${browserOut.consoleMessages.join('\n')}\n`);
  }
  if (browserOut?.pageErrors?.length) {
